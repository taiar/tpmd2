...
void insertRegister(unsigned int id, char *name, char *email)
{
 ...
 insertCell(&list, (void *) &reg, sizeof(TRegister));
}
...
int main()
{
 ...
 reg = retrieveData(&list, compare, (void *) "João Henrique Pato");
 printRegister(reg);
  
 //Procura por Fernanda e imprime os seus dados.
 reg = retrieveData(&list, compare, (void *) "Fernanda Lara dos Santos");
 printRegister(reg);
  
 //Remove João.
 removeCell(&list, compare, (void *) "João Henrique Pato");
 ...
 //Procura por Pedro.
 reg = retrieveData(&list, compare, (void *) "Pedro Malta Ribeiro");
 printRegister(reg);
